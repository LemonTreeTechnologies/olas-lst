...
multisigGuardAddress=$(jq -r '.multisigGuardAddress' $globals)
...
constructorArgs="$multisigGuardAddress $proxyData"
...
multisigGuardProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#multisigGuardProxyAddress}
...
echo "$(jq '. += {"multisigGuardProxyAddress":"'$multisigGuardProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$multisigGuardProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $multisigGuardProxyAddress${reset}"
