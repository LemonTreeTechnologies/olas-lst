...
# Sets Safe multisig implementation addresses on ExternalStakingDistributorProxy.
#
# This MUST be run right after upgrading the ExternalStakingDistributor implementation: proxies deployed by an
# earlier implementation have no safeMultisig / safeSetupHelper in storage, and service creation reverts with
# ZeroAddress until this script is run.
...
for implementation in $gnosisSafeMultisigImplementationAddress; do
...
echo "${green}Change multisig implementations in ExternalStakingDistributorProxy${reset}"
castArgs="$externalStakingDistributorProxyAddress changeMultisigImplementations(address,address) $gnosisSafeMultisigImplementationAddress $safeSetupHelperAddress"
echo $castArgs
