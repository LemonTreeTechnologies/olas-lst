...
constructorArgs="$externalStakingDistributorAddress $proxyData"
...
externalStakingDistributorProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#externalStakingDistributorProxyAddress}
...
echo "$(jq '. += {"externalStakingDistributorProxyAddress":"'$externalStakingDistributorProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$externalStakingDistributorProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $externalStakingDistributorProxyAddress${reset}"
