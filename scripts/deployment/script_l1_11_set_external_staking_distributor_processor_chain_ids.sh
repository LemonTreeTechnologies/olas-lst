...
l2ChainId=$(jq -r '.chainId' $globalsL2)
...
echo "${green}Set ExternalStakingDistributor chain Ids${reset}"
castArgs="$depositoryProxyAddress setExternalStakingDistributorChainIds(uint256[],address[]) [$l2ChainId] [$externalStakingDistributorProxyAddress]"
echo $castArgs
